...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      new Paragraph({
        heading: HeadingLevel.HEADING_1,
        children: [new TextRun({ text: "三、建议（请至少提供一个案例）" })]
      }),

      new Paragraph({
        heading: HeadingLevel.HEADING_2,
        children: [new TextRun({ text: "（一）发展县域经济与新业态，拓宽返乡就业渠道" })]
      }),
      new Paragraph({
        heading: HeadingLevel.HEADING_2,
        children: [new TextRun({ text: "（二）深化「企校双制」培训，适配转型需求" })]
      }),
      new Paragraph({
        heading: HeadingLevel.HEADING_2,
        children: [new TextRun({ text: "（三）深化社保制度改革与政策创新，提高务工稳定性与合规性" })]
      }),
      new Paragraph({
        heading: HeadingLevel.HEADING_2,
        children: [new TextRun({ text: "（四）强化「政银保」协同发力，化解高龄用工风险与结构性断层" })]
      }),
      new Paragraph({
        heading: HeadingLevel.HEADING_2,
        children: [new TextRun({ text: "（五）其他，请补充" })]
      }),
    ]
  }]
});
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync("/home/admin/openclaw/workspace/调研提纲_贾珍珍_修改版.docx", buffer);
  console.log("文档已生成：/home/admin/openclaw/workspace/调研提纲_贾珍珍_修改版.docx");
});
