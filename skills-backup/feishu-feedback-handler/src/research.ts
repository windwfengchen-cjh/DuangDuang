...
import { getTenantAccessToken, sendFeishuMessage, sendFeishuPostMessage } from './utils';
 ...
 * 获取格式化的问题文本（替换变量）
 */
export function formatQuestion(questionTemplate: string, title: string, requesterName: string): string {
...
export interface ResearchSession {
  ...
  // === 模式配置 ===
  // 调研模式: 'auto_collect' | 'qa' | 'hybrid'（默认hybrid）
  mode: ResearchMode;
  // 旧字段兼容，等同于 mode === 'auto_collect'
...
export function createResearchSession(
  ...
  requirementPersonId: string,
  requirementPersonName: string,
  bossId: string,
  recordId?: string,
  mode: ResearchMode = 'hybrid',  // 默认使用混合模式
  autoCollectHours: number = 24   // 默认自动收集24小时
): ResearchSession {
  const now = new Date();
...
开始收集需求信息... 🚀`
  ...
  await sendFeishuMessage(chatId, welcomeMsg, token);
  ...
  if (session.auto_collect_mode) {
    ...
    // 问答模式：发送第一个问题
    const firstQuestion = RESEARCH_QUESTIONS[0];
    await sendFeishuMessage(chatId, firstQuestion.question, token);
    session.status = 'collecting';
...
export async function handleResearchMessage(
  ...
  // 保存消息到记录（所有消息都保存）
  // 使用 Date.now() 而不是 event.create_time，避免飞书事件时间戳错误
  ...
  if (isCancelCommand) {
    ...
    if (isRequirementPerson || senderId === session.boss_id) {
      ...
      saveResearchSession(session);
      await sendFeishuMessage(chatId, '❌ 需求调研已取消。如需重新开始，请重新触发需求跟进流程。', token);
  ...
  if (isCompleteCommand || isExpired) {
    ...
    if (isCompleteCommand) {
      ...
      await sendFeishuMessage(chatId, '⏰ 自动收集时间已到，正在生成PRD文档...', token);
    }
  ...
  if (session.auto_collect_mode) {
    ...
    if (session.message_count % 10 === 0) {
      await sendFeishuMessage(chatId, `✅ 已收集 ${session.message_count} 条消息，继续收集需求信息...`, token);
    }
  ...
  // 保存当前问题的回答
  const currentQuestion = RESEARCH_QUESTIONS[session.current_question_idx];
  if (currentQuestion) {
  ...
  if (session.current_question_idx < RESEARCH_QUESTIONS.length) {
    const nextQuestion = RESEARCH_QUESTIONS[session.current_question_idx];
    await sendFeishuMessage(chatId, nextQuestion.question, token);
    saveResearchSession(session);
...
async function generatePRD(session: ResearchSession, token: string): Promise<void> {
  ...
  await sendFeishuMessage(chatId, '📝 正在根据收集的信息生成PRD文档...', token);
  ...
  await sendFeishuMessage(chatId, summaryMsg, token);
