...
class RequirementFollowWorkflow {
    ...
    async startWorkflow(params) {
        ...
        try {
            ...
            (0, research_1.createResearchSession)(chatInfo.chat_id, params.requirementContent.slice(0, 100), // 需求标题（取内容前100字）
            params.requesterId, params.requesterName, this.config.bossId, recordId, 'hybrid', // 默认使用混合模式（自动收集 + 问答）
            24 // 24小时自动收集截止时间
            );
            sessionCreated = true;
            console.log(`📝 调研会话已创建并保存到: ~/.openclaw/feishu/research/${chatInfo.chat_id}.json`);
            // 启动调研（发送欢迎消息和第一个问题）
            await (0, research_1.startResearch)(chatInfo.chat_id, token);
    ...
    async addMembersWithFallback(chatId, userIds, userNames, chatName, requesterId) {
    ...
    async sendRequesterJoinNotice(chatId, requesterName, token) {
        try {
     ...
     */
    startChatPolling(chatId, intervalMs = 5000) {
        // 如果已有轮询，先停止
        this.stopChatPolling(chatId);
        console.log(`[${chatId}] 启动消息轮询，间隔 ${intervalMs}ms`);
        const poll = async () => {
            try {
                const session = (0, research_1.loadResearchSession)(chatId);
                ...
                await (0, research_1.pollResearchChat)(chatId, this.api, {
...
function parseCommandLineArgs() {
    ...
    if (args.includes('--daemon') || args.includes('-d')) {
        const daemonParams = { mode: 'daemon' };
 ...
 */
async function readStdinParams() {
 ...
 * 运行守护进程模式
 */
async function runDaemonMode(pollInterval = 5000) {
    console.log('='.repeat(60));
    ...
    const api = new api_1.FeishuAPI({ appId: config.appId, appSecret: config.appSecret });
    // 显示当前活跃会话
    const activeSessions = (0, research_1.getActiveResearchSessions)();
    ...
    console.log('');
    // 启动全局轮询
    const polling = (0, research_1.startResearchPolling)(api, {
