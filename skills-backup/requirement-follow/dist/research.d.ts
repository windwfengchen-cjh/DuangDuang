...
interface PollOptions {
    intervalMs?: number;
    onMessage?: (chatId: string, result: any) => void;
    onError?: (chatId: string, error: Error) => void;
}
/**
 * 获取所有活跃的调研会话
 */
export declare function getActiveResearchSessions(): ResearchSession[];
/**
 * 轮询单个调研群的消息
 * @param chatId 群ID
 ...
 * @returns 是否启动成功
 */
export declare function pollResearchChat(chatId: string, api: FeishuAPI, options?: PollOptions): Promise<boolean>;
/**
 * 启动持续轮询所有活跃调研群
 * @param api FeishuAPI 实例
 * @param options 轮询选项
 * @returns 停止轮询的函数
 */
export declare function startResearchPolling(api: FeishuAPI, options?: PollOptions): {
    stop: () => void;
    isRunning: () => boolean;
};
